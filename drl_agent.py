...
from data_handler import process_historical_data, get_normalized_market_data, get_available_assets
...
class GymTradingEnv(gym.Env):
    ...
    def __init__(self, data, symbol, state_size=512, initial_balance=100):
        ...
        self.observation_space = spaces.Box(
            low=-1, high=1, shape=(SEQUENCE_LENGTH * self.data.shape[1],), dtype=np.float32
        )
    ...
    def reset(self):
        ...
        self.current_step = SEQUENCE_LENGTH
        return self.data[self.current_step - SEQUENCE_LENGTH:self.current_step].flatten()
    ...
    def step(self, action):
        ...
        obs = self.data[self.current_step - SEQUENCE_LENGTH:self.current_step].flatten()
...
class DRLSuperAgent:
    ...
    def __init__(self, state_size=512, action_space_type="discrete", env=None):
        ...
        self.env = env or DummyVecEnv([lambda: GymTradingEnv(
            data=None,  # Placeholder, deve essere passato un ambiente valido
            initial_balance=10000
        )])
        self.drl_agent = DRLAgent(state_size=state_size)

        # Seleziona automaticamente l'algoritmo in base al tipo di spazio di azione
    ...
    def _select_algorithm(self):
        ...
        if isinstance(space, spaces.Box):
            ...
            raise ValueError(
                f"Tipo di spazio di azione non supportato: {type(self.env.action_space)}"
            )
